...
int checkInput(char p[], int mode, int length){
	...
	while(1){
		while (kbhit()){
			...
			if (((kb_hit >= 65 && kb_hit <= 90) || (kb_hit >= 97 && kb_hit <= 122) || (kb_hit >= 48 && kb_hit <=57)) && count < length){
				...
				input[count] = (char)kb_hit;
				count++;	 									//gan truoc roi tang sau
			}
			if(kb_hit == enter){								//enter
				strncpy(p,input,length -1);
...
int checkEdit(char p[], node &editnode, int vitri, int mORe){
	...
	while(1){
		while (kbhit()){
			...
			if (((kb_hit >= 65 && kb_hit <= 90) || (kb_hit >= 97 && kb_hit <= 122)) || (kb_hit >=49 &&kb_hit <=57) && count < 200){
				if(count >= (sodong*70) && count % 70 == 0) {
					gotoxy(64,wherey()+1);
					sodong++;
				}
...
void chinhSuaMean(node repairWord){
	...
	for(int i = 1;i<=vitriMean[0];i++){
		if(ycol2 == vitriMean[i]){  // lay vi tri cua con tro => vi tri cua mean trong list mean
			...
			for(p = repairWord->newWord.meanFirst ; p!=NULL ; p = p->next){
				if(dem== i){
					for(int k = vitriMean[i];k<=(vitriMean[i]+strlen(p->wordMean)/70) ;k++){
						clearOneLine(63,k,76);	// xoa mean cu
					}
					char newMean[200]="";
...
void chinhSuaEx( node repairWord){
	for(int i = 1;i<= vitriEx[0];i++){
		if(ycol2 == vitriEx[i]){	//lay vi tri cua mean tu ycol2 => vi tri cua Ex trong mang ex[]
			for(int k = vitriEx[i];k <= (vitriEx[i] + strlen(repairWord->newWord.example[i-1])/76);k++) clearOneLine(64,k,76);
			gotoxy(64,vitriEx[i]);
			char save[200]="";
			checkEdit(save,repairWord,i-1,2); // i-1 vì mang vitriEx bat dau tu 1 con mang ex bat dau tu 0
			if(strlen(save)==0){
				gotoxy(64,vitriEx[i]);
				for(int k = 0; k<strlen(repairWord->newWord.example[i-1]); k++){
					if( k > 0 && k%70 == 0) gotoxy(64,wherey()+1);
					printf("%c",repairWord->newWord.example[i-1][k]);
				}
			}
			else {
				editFlag =1 ;
